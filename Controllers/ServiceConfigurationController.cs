    ...
    public class ServiceConfigurationController : Controller
    {
        ...
        public JsonResult Create(ServiceConfiguration model)
        {
            try
            {
                ...
                if (existingService)
                {
                    return Json(new { success = false, message = "Bu servis kodu zaten kullanýlýyor." });
                }
                ...
                if (ModelState.IsValid)
                {
                    ...
                    return Json(new { success = true, message = "Servis konfigürasyonu baþarýyla oluþturuldu.", data = model });
                }
        ...
        public JsonResult Edit(ServiceConfiguration model)
        {
            try
            {
                ...
                if (existingService)
                {
                    return Json(new { success = false, message = "Bu servis kodu baþka bir kayýt tarafýndan kullanýlýyor." });
                }
                ...
                if (ModelState.IsValid)
                {
                    ...
                    return Json(new { success = true, message = "Servis konfigürasyonu baþarýyla güncellendi.", data = existing });
                }
        ...
        public JsonResult Delete(int id)
        {
            try
            {
                ...
                if (personnelCount > 0)
                {
                    return Json(new { success = false, message = $"Bu servis {personnelCount} personel tarafýndan kullanýlýyor. Önce personel atamalarýný kaldýrýn." });
                }

                db.ServiceConfigurations.Remove(service);
                db.SaveChanges();

                return Json(new { success = true, message = "Servis konfigürasyonu baþarýyla silindi." });
            }
            catch (Exception ex)
            {
                return Json(new { success = false, message = ex.Message });
        ...
        public JsonResult CreateDefaultServices()
        {
            try
            {
                ...
                {
                    return Json(new { success = false, message = "Zaten servis konfigürasyonlarý mevcut." });
                }

                var currentUserId = GetCurrentUserId();
                var defaultServices = new List<ServiceConfiguration>
                {
                    new ServiceConfiguration { ServiceCode = "IT", ServiceName = "IT Departmaný", Description = "Bilgi iþlem ve teknoloji departmaný", ColorCode = "#3B82F6", IconClass = "computer", SortOrder = 1, CreatedBy = currentUserId },
                    new ServiceConfiguration { ServiceCode = "HR", ServiceName = "Ýnsan Kaynaklarý", Description = "Ýnsan kaynaklarý departmaný", ColorCode = "#10B981", IconClass = "users", SortOrder = 2, CreatedBy = currentUserId },
                    new ServiceConfiguration { ServiceCode = "Finance", ServiceName = "Finans", Description = "Mali iþler departmaný", ColorCode = "#F59E0B", IconClass = "money", SortOrder = 3, CreatedBy = currentUserId },
                    new ServiceConfiguration { ServiceCode = "Marketing", ServiceName = "Pazarlama", Description = "Pazarlama ve satýþ departmaný", ColorCode = "#8B5CF6", IconClass = "megaphone", SortOrder = 4, CreatedBy = currentUserId },
                    new ServiceConfiguration { ServiceCode = "Operations", ServiceName = "Operasyon", Description = "Operasyonel iþlemler departmaný", ColorCode = "#6B7280", IconClass = "settings", SortOrder = 5, CreatedBy = currentUserId }
                };

                db.ServiceConfigurations.AddRange(defaultServices);
                db.SaveChanges();

                return Json(new { success = true, message = "Varsayýlan servis konfigürasyonlarý oluþturuldu." });
            }
            catch (Exception ex)
            {
                return Json(new { success = false, message = ex.Message });
