            ...
            using (var transaction = db.Database.BeginTransaction())
            {
                try
                {
                    ...
                    if (user != null)
                    {
                        ...
                        // =======================================================
                        // SQL Script ile aynı sırayı takip ederek silme işlemleri
                        // =======================================================

                        // 1. PERMISSION & ROLE SİSTEMİ TEMİZLİĞİ
                        ...
                        // 2. User Role ilişkileri siliniyor
                        ...
                        // 3. NOTIFICATION SİSTEMİ TEMİZLİĞİ
                        ...
                        // 4. CHAT SİSTEMİ TEMİZLİĞİ
                        // Chat Group üyelikleri siliniyor
                        ...
                        // 5. Chat mesajları soft delete yapılıyor
                        var chatMessages = db.ChatMessages.Where(cm => cm.UserId == id && !cm.IsDeleted).ToList();
                        foreach (var message in chatMessages)
                        {
                            message.IsDeleted = true;
                            message.DeletedAt = DateTime.Now;
                            message.DeletedBy = id;
                        }

                        // 6. Oluşturulan Chat Groups siliniyor
                        ...
                        // 6.1. ChatGroupUsers tablosu (SQL scriptinde var)
                        try
                        {
                            // Bu tablo DbSet'te tanımlı olmayabilir, ham SQL kullan
                            db.Database.ExecuteSqlCommand("DELETE FROM ChatGroupUsers WHERE User_Id = @p0", id);
                        }
                        catch (Exception ex)
                        {
                            System.Diagnostics.Debug.WriteLine($"ChatGroupUsers silme hatası: {ex.Message}");
                        }

                        // 6.2. ChatGroupUser1 tablosu (SQL scriptinde var)
                        try
                        {
                            // Bu tablo DbSet'te tanımlı olmayabilir, ham SQL kullan
                            db.Database.ExecuteSqlCommand("DELETE FROM ChatGroupUser1 WHERE User_Id = @p0", id);
                        }
                        catch (Exception ex)
                        {
                            System.Diagnostics.Debug.WriteLine($"ChatGroupUser1 silme hatası: {ex.Message}");
                        }

                        // 7. Oluşturulan Chat kayıtları siliniyor
                        ...
                        // 8. TASK YÖNETİMİ SİSTEMİ TEMİZLİĞİ
                        // Todo Items siliniyor
                        ...
                        // 10. SERVİS YÖNETİMİ SİSTEMİ TEMİZLİĞİ
                        // Service Personnel kayıtlarını sil
                        ...
                        // 11. Overtime Service Personnel kayıtlarını sil
                        ...
                        // 12. DİLEK & ÖNERİ SİSTEMİ TEMİZLİĞİ
                        // DilekOneri kayıtlarını sil (Username ile eşleşen)
                        ...
                        // 13. Suggestion Complaint kayıtlarını sil
                        ...
                        // 14. ANKET SİSTEMİ TEMİZLİĞİ
                        // Answer Options siliniyor
                        ...
                        foreach (var answer in userAnswers)
                        {
                            ...
                            // 15. Survey Answers siliniyor
                            db.Answers.Remove(answer);
                        }

                        // 16. REZERVASYON SİSTEMİ TEMİZLİĞİ
                        // Rezervasyon kayıtları siliniyor
                        ...
                        // 17. Meeting Room Reservation kayıtları siliniyor
                        ...
                        // 18. PERSONEL TAKİP SİSTEMİ TEMİZLİĞİ
                        // Daily Mood kayıtları siliniyor
                        ...
                        // 19. Leave Request kayıtları siliniyor
                        ...
                        // 20. Beyaz Tahta Entry kayıtları siliniyor
                        ...
                        // 21. ZİYARETÇİ TAKİP SİSTEMİ TEMİZLİĞİ
                        // Visitor Entry kayıtları siliniyor (FullName ile eşleşen)
                        try
                        {
                        ...
                        // 22. Late Arrival Report kayıtları siliniyor (FullName ile eşleşen)
                        try
                        {
                        ...
                        // 23. ŞİFRE SIFIRLAMA SİSTEMİ TEMİZLİĞİ
                        // Password Reset Request kayıtları siliniyor
                        ...
                        // 25. DÖKÜMAN YÖNETİMİ TEMİZLİĞİ
                        // Dokümantasyon kayıtları siliniyor (Username ile eşleşen)
                        var dokumentasyons = db.Dokumantasyons.Where(d => d.Username == user.Username).ToList();
                        ...
                        try
                        {
                            ...
                            DynamicPermissionHelper.InvalidateUserCache(id);
                        ...
                        TempData["SuccessMessage"] = $"Kullanıcı '{user.Username}' ve tüm ilişkili kayıtları başarıyla silindi.";
                    }
                    else
                    {
