...
namespace MZDNETWORK.Helpers
{
    ...
    public static class DynamicPermissionHelper
    {
        ...
        /// <summary>
        /// HasPermission alias for CheckPermission (API consistency)
        /// </summary>
        public static bool HasPermission(int userId, string permissionPath, string action = "View")
        {
            return CheckPermission(userId, permissionPath, action);
        }
        ...
        private static bool CheckPermissionFromData(UserPermissionData permissions, string permissionPath, string action)
        {
            ...
            if (!string.Equals(action, "View", StringComparison.OrdinalIgnoreCase))
            {
                ...
                if (permissions.Permissions.ContainsKey(actionAppendedPath))
                {
                    return CheckActionPermission(permissions.Permissions[actionAppendedPath], action);
                }
            }
